...
def init_bigquery_table():
	...
	date_field.name = 'city'
	date_field.type = 'string'
	date_field.mode = 'required'
	table_schema.fields.append(date_field)

	date_field = bigquery.TableFieldSchema()
	date_field.name = 'state'
	date_field.type = 'string'
	date_field.mode = 'required'
	table_schema.fields.append(date_field)

	date_field = bigquery.TableFieldSchema()
	date_field.name = 'metro'
	date_field.type = 'string'
	date_field.mode = 'required'
	table_schema.fields.append(date_field)

	date_field = bigquery.TableFieldSchema()
	date_field.name = 'county'
	date_field.type = 'string'
	date_field.mode = 'required'
	table_schema.fields.append(date_field)
...
def parse_line(line):
	...
	start_month_index = 43; # 2015-01 is on column 43
...
def run(argv=None):	
	...
	with beam.Pipeline(options=pipeline_options) as p:
		...
		table_name = "juan-chujun:zillow.Rental_Price_5Bedroom" # format: project_id:dataset.table
		table_schema = init_bigquery_table()
    
		lines = p | 'ReadFile' >> beam.io.ReadFromText('gs://juan-chujun/zillow/Zip_MedianRentalPrice_5BedroomOrMore.csv')
