...
def run(argv=None):	
	...
	pipeline_args.extend([	
      ...
      '--job_name=rentals-2bedroom' # assign descriptive name to this job, all in lower case letters
	])
	...
	with beam.Pipeline(options=pipeline_options) as p:
		...
		table_name = "juan-chujun:zillow.Rental_Price_2Bedroom" # format: project_id:dataset.table
		table_schema = init_bigquery_table()
    
		lines = p | 'ReadFile' >> beam.io.ReadFromText('gs://juan-chujun/zillow/Zip_MedianRentalPrice_2Bedroom.csv')
